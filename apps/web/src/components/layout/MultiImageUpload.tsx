...
import { IconPlus, IconStar, IconTrash } from '@/icons';
...
export function MultiImageUpload({ value = [], onChange, max = 8 }: Props) {
  ...
  const readFiles = (files: FileList | null) => {
    ...
    Promise.all(
      list.map((f) => new Promise<string>((res, rej) => {
        const r = new FileReader();
        r.onload = () => res(r.result as string);
        r.onerror = rej;
        r.readAsDataURL(f);
      }))
    ).then((urls) => onChange([...value, ...urls].slice(0, max)));
  };
            ...
            {value.map((src, i) => {
              ...
              return (
                <div
                  ...
                  className={`group relative aspect-square bg-stone-50 border-2 overflow-hidden cursor-move transition-all ${isCover ? 'border-bc-green ring-2 ring-bc-green/20' : 'border-stone-200'} ${isDragging ? 'opacity-40 scale-95' : ''} ${isDropTarget ? 'ring-2 ring-bc-red border-bc-red' : ''}`}
                >
