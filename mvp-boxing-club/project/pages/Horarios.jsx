// ============================================================
// Horarios — reads from ScheduleService, adapts to data
// ============================================================
...
function HorariosPage() {
  ...
  }, [items, filterCat, search, categories]);

  // Group by time/day for grid
  const grid = React.useMemo(() => {
    if (!filtered.length) return { times: [], byCell: {} };
    const timeSet = new Set();
    const byCell = {}; // time|day → items[]
    filtered.forEach((it) => {
      timeSet.add(it.time);
  ...
  return (
    <section data-screen-label="Horários" className="relative pt-32 pb-24 sm:pt-40 sm:pb-32 bg-cream overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
        ...
        {/* Filters */}
        <div className="mb-5 flex flex-wrap items-center gap-2 animate-reveal-up" style={{ animationDelay: '100ms' }} data-comment-anchor="66548338dd-div-70-9">
          ...
          <button onClick={() => setFilterCat('all')} className={`px-3 py-2 text-[11px] uppercase tracking-[0.2em] font-semibold border ${filterCat === 'all' ? 'bg-ink text-white border-ink' : 'bg-white text-stone-700 border-stone-300 hover:border-ink'}`}>Todas</button>
          {categories.map((c) =>
          <button key={c.id} onClick={() => setFilterCat(c.id)} className={`px-3 py-2 text-[11px] uppercase tracking-[0.2em] font-semibold border inline-flex items-center gap-2 ${filterCat === c.id ? 'text-white border-transparent' : 'bg-white text-stone-700 border-stone-300 hover:border-ink'}`} style={filterCat === c.id ? { background: c.color } : {}}>
        ...
        <ScheduleSkeleton /> :
        filtered.length === 0 ?
        <EmptyState title="Nenhuma aula encontrada" message="Tente ajustar os filtros ou a busca." icon={<IconCalendar size={28} />} /> :
        ...
        <>
            ...
            <div className="hidden md:block animate-reveal-up" style={{ animationDelay: '200ms' }}>
              <div className="border border-stone-200 bg-white shadow-[0_30px_80px_-40px_rgba(13,107,58,0.25)] overflow-hidden">
                <div className="grid bg-ink" style={{ gridTemplateColumns: '110px repeat(6, minmax(0, 1fr))' }}>
                  <div className="px-4 py-4 border-r border-white/10 flex items-center justify-center"><IconClock size={16} className="text-stone-400" /></div>
                  {DAYS_FULL.map((d, i) =>
              ...
              <div key={time} className="grid border-t border-stone-200" style={{ gridTemplateColumns: '110px repeat(6, minmax(0, 1fr))' }}>
                  ...
                  if (cellItems.length === 0) {
                    return <div key={d} className="border-r last:border-r-0 border-stone-200 min-h-[68px] bg-[linear-gradient(135deg,transparent_46%,#f1efe7_47%,#f1efe7_53%,transparent_54%)] bg-[size:14px_14px]" />;
                  }
                  return (
                    <div key={d} className="border-r last:border-r-0 border-stone-200 min-h-[68px] p-1.5 flex flex-col gap-1">
                        ...
                        return (
                          <div key={it.id} className="flex-1 rounded-sm px-2 py-1.5 flex flex-col items-center justify-center text-center text-white transition-transform hover:scale-[1.03] cursor-pointer min-h-[36px]" style={{ background: cat?.color || '#0e1410' }} title={`${cat?.label || ''} · ${it.teacher || ''}${it.notes ? ' · ' + it.notes : ''}`}>
            ...
            <div className="md:hidden space-y-3">
              ...
              return (
                <div key={day} className="border border-stone-200 bg-white p-5 animate-reveal-up" style={{ animationDelay: `${di * 60 + 100}ms` }}>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-display text-2xl tracking-wider text-ink">{day.toUpperCase()}</h3>
                      <span className="text-xs text-stone-500 uppercase tracking-widest">{dayItems.length === 0 ? 'Sem aulas' : `${dayItems.length} aula${dayItems.length > 1 ? 's' : ''}`}</span>
                    </div>
                    {dayItems.length === 0 ?
                      ...
                      return (
                        <li key={it.id} className="flex items-center gap-3">
                              ...
                              <span className="flex-1 px-3 py-2 text-xs font-display tracking-widest uppercase text-white" style={{ background: cat?.color || '#0e1410' }}>
                                {cat?.label || it.category}{it.notes ? ` · ${it.notes}` : ''}
                              </span>
                            </li>);
        ...
        <p className="text-center text-xs text-stone-500 mt-8 uppercase tracking-widest">* Horários sujeitos a alteração. Confirme pelo Instagram @team_boxing_club.</p>
      </div>
    </section>);
...
Object.assign(window, { HorariosPage });